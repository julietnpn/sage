...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		#csv_import2(path2, 'UPDATE', user2)
		#csv_import3(path3, user3)
		#csv_import1(path1, 'INSERT', user1)
		#csv_import2(path2, 'INSERT', user2)
		#csv_import3(path3, 'INSERT', user3)
...
def csv_import2(path,trans_type, user):############################serotiny, degree_of_serotiny, allelochemicals######
	with open(path) as f:
		...
		for i,plant in enumerate(reader):#reader:
			...
			transactions = Transactions.objects.create(users_id=user.id, transaction_type='INSERT', ignore=False)# not always Update
			actions = []

			try:
				plant_id = Plant.objects.filter(genus=plant['Scientific Name'].split()[0]).first().id
				transactions.plants_id=plant_id
				transactions.transaction_type = 'UPDATE'
				transactions.save()
			except AttributeError:
				pass
