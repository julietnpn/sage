...
for plant in read_plants(r"C:\Users\John\Dropbox\research\CyberSEES\usda.orange.for_import.csv"):
    ...
    actions.append(Actions(transaction=transaction, action_type="INSERT", property='genus', value=plant['Scientific Name'].split()[0]))
    actions.append(Actions(transaction=transaction, action_type="INSERT", property='species', value=plant['Scientific Name'].split()[1]))

    # 3. add field for Family Common Name
    if plant['Family Common Name'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='family_common_name', value=plant['Family Common Name'].strip()))

    # 4. add field for Family
    if plant['Family'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='family', value=plant['Family'].strip()))
    ...
    for duration in durations:
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='duration', value=duration_id))
    ...
    #    actions.append(Actions(transaction=transaction, action_type="INSERT", property='layer', value=layer_id))

    # 7. Urls found in Fact Sheets and Plant Guides should be stored into the url tags
    if plant['Fact Sheets'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='url_tags', value=plant['Fact Sheets'].strip()))
    if plant['Plant Guides'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='url_tags', value=plant['Plant Guides'].strip()))
    ...
    if plant['Cultivar Name'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='variety', value=plant['Cultivar Name'].strip()))
    ...
    for season in seasons:
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='active_growth_period', value=active_growth_period_id))
    ...
    if plant['Bloat'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='livestock_bloat_id', value=livestock_bloat_id))
    ...
    if plant['Coppice Potential'].strip() == 'Yes':
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='raw_materials_prod', value=raw_materials_prod_id))
    ...
    if plant['Fire Resistance'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='fire_tol_id', value=fire_tol_id))
    ...
    if plant['Flower Color'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='flower_color', value=flower_color_id))
    if plant['Fruit Color'].strip():
        fruit_color_id = FruitColor.query.filter_by(value=plant['Fruit Color'].strip().lower()).first().id
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='fruit_color', value=fruit_color_id))
    if plant['Foliage Color'].strip():
        foliage_color_id = FoliageColor.query.filter_by(value=plant['Foliage Color'].strip().lower()).first().id
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='foliage_color', value=foliage_color_id))
    ...
    if plant['Flower Conspicuous'].strip() == 'Yes' or plant['Fruit Conspicuous'].strip() == 'Yes':
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='cultural_and_amenity_prod', value=cultural_and_amenity_prod_id))
    ...
    if plant['Foliage Porosity Summer'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='canopy_density', value=canopy_density_id))

    # 19. Height, mature maps to height at maturity in feet (no unit change)
    if plant['Height, Mature (feet)'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='height_at_maturity', value=float(plant['Height, Mature (feet)'].strip())))
    ...
    if plant['Leaf Retention'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='leaf_retention', value=leaf_retention_id))
    ...
    if plant['Known Allelopath'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='allelopathic_id', value=allelopathic_id))
    if plant['Lifespan'].strip():
        lifespan_id = Lifespan.query.filter_by(value=plant['Lifespan'].strip().lower()).first().id
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='lifespan_id', value=lifespan_id))
    ...
    if plant['Nitrogen Fixation'].strip() in ['Low', 'Medium', 'High']:
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='mineral_nutrients_prod', value=mineral_nutrients_prod_id))
    ...
    if plant['Toxicity'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='toxicity_id', value=toxicity_id))
    ...
    if plant['Fertility Requirement'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='fertility_needs', value=fertility_needs_id))
    ...
    if plant['Moisture Use'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='water_needs', value=water_needs_id))

    # 29. ph Maximum and ph Minimum map directly to our db (need to create max and min values though).
    if plant['pH (Minimum)'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='pH_min', value=float(plant['pH (Minimum)'].strip())))
    if plant['pH (Maximum)'].strip():
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='pH_max', value=float(plant['pH (Maximum)'].strip())))
    ...
    if plant['Salinity Tolerance'].strip():
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='salt_tol_id', value=salt_tol_id))
    if plant['Shade Tolerance'].strip():
        ...
        for shade_tol in shade_tols:
            ...
            actions.append(Actions(transaction=transaction, action_type="INSERT", property='shade_tol', value=shade_tol_id))
    ...
    if plant['Fruit/Seed Period End'].strip():
        ...
        for season in seasons:
            ...
            actions.append(Actions(transaction=transaction, action_type="INSERT", property='harvest_period', value=harvest_period_id))
    ...
    if plant['Berry/Nut/Seed Product'].strip() == 'Yes':
        ...
        actions.append(Actions(transaction=transaction, action_type="INSERT", property='food_prod', value=food_prod_id))
